        ...
        return (
            <div style={{ display: "flex", flexDirection: "column", gap: 12, padding: "14px 14px 0", minHeight: 0, height: "100%" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8, flex: 1, minHeight: 0, overflowY: "auto", paddingRight: 6, paddingBottom: 120 }}>
                    {pending.map((t, idx) => (
                        <div key={t.id} style={{ padding: "12px", border: `1px solid ${THEME.colors.border}`, borderRadius: 16, background: "rgba(255,255,255,0.02)", display: "flex", flexDirection: "column", gap: 8 }}>
                            <div style={{ display: "flex", gap: 8 }}>
                                <input 
                                    value={t.title}
                                    onChange={(e) => {
                                        const newPending = [...pending];
                                        newPending[idx].title = e.target.value;
                                        setPending(newPending);
                                    }}
                                    placeholder="Título"
                                    style={{ flex: 2, background: "rgba(255,255,255,0.05)", border: "none", borderRadius: 8, padding: "8px 12px", color: "white", fontSize: 13, fontWeight: 700, outline: "none" }}
                                />
                                <input 
                                    value={t.artist}
                                    onChange={(e) => {
                                        const newPending = [...pending];
                                        newPending[idx].artist = e.target.value;
                                        setPending(newPending);
                                    }}
                                    placeholder="Artista"
                                    style={{ flex: 1, background: "rgba(255,255,255,0.05)", border: "none", borderRadius: 8, padding: "8px 12px", color: THEME.colors.text.muted, fontSize: 12, outline: "none" }}
                                />
                            </div>
                            <div style={{ fontSize: 10, color: THEME.colors.brand.cyan, fontWeight: 800, paddingLeft: 4, opacity: 0.8 }}>
                                {t.bpm} BPM • {t.key} • {formatDuration(t.duration_ms)} • {t.file.name}
                            </div>
                        </div>
                    ))}
                </div>
