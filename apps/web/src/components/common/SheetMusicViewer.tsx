...
export const SheetMusicViewer: React.FC<SheetMusicViewerProps> = ({ items, onClose }) => {
    ...
    // ── AUTO-PAGE TURN LOGIC (Nivel 4) ──
    useEffect(() => {
        if (!playing || !ct?.markers) return;

        // Buscamos si hay un marcador que coincida con la posición actual 
        // y que tenga el formato "PAGE_X" o "PÁGINA_X"
        const pageMarkers = ct.markers
            .filter(m => /^(PAGE|PAGINA|PÁGINA)_\d+$/i.test(m.comment))
            .sort((a, b) => b.posMs - a.posMs); // Orden descendente para encontrar el más cercano pasado

        const activeMarker = pageMarkers.find(m => pos >= m.posMs);
        
        if (activeMarker) {
            const pageNum = parseInt(activeMarker.comment.split("_")[1]);
            const targetIndex = pageNum - 1;
            if (targetIndex >= 0 && targetIndex < items.length && targetIndex !== currentIndex) {
                console.log(`[SheetMusic] 📄 Auto-switching to page ${pageNum} due to marker at ${activeMarker.posMs}ms`);
                setCurrentIndex(targetIndex);
            }
        }
    }, [pos, playing, ct?.markers, items.length, currentIndex]);
