...
export function useAudio() {
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            ...
            const progressPct = (currentMs / trackEnd) * 100;

            // ── PRE-CARGA PROACTIVA (Nivel 2) ──
            // Si pasamos el 70% del tema, ya empezamos a calentar el otro canal
            if (progressPct > 70 && !manualPauseTransitionRef.current) {
                const nextIndex = resolveNextTrackIndex(usePlayerStore.getState());
                if (nextIndex !== null && nextIndex !== ci) {
                    const nextTrack = pQueue[nextIndex];
                    const inactiveAudio = activeChannel.current === "A" ? channelBRef.current : channelARef.current;
                    
                    // Solo cargamos si el src es diferente (evitar re-carga infinita)
                    const nextUrl = getTrackUrl(nextTrack);
                    if (inactiveAudio && inactiveAudio.src !== nextUrl && !playbackQueueRef.current) {
                        console.log(`[useAudio] 🚀 Proactive Pre-load started for: ${nextTrack.title}`);
                        AudioStreamerService.fetchWithProgress(nextUrl, (p) => updateDownload(nextTrack.id, p), nextTrack.id)
                            .then(objectUrl => {
                                if (inactiveAudio.src !== objectUrl) {
                                    inactiveAudio.src = objectUrl;
                                    inactiveAudio.load();
                                    (inactiveAudio as any)._lastTrackId = nextTrack.id;
                                }
                            });
                    }
                }
