        ...
        <div style={{
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
                <main style={{ flex: 1, display: "flex", flexDirection: "column", padding: "24px", overflowY: "auto", gap: 32 }}>
                    ...
                    <header style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 20 }}>
                        <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                                {ct && (
                                    <>
                                        <span style={{ fontSize: 10, padding: "4px 10px", borderRadius: 4, background: THEME.colors.brand.cyan + "15", color: THEME.colors.brand.cyan, fontWeight: 800 }}>{ct.bpm} BPM</span>
                                        <span style={{ fontSize: 10, padding: "4px 10px", borderRadius: 4, background: THEME.colors.brand.violet + "15", color: THEME.colors.brand.violet, fontWeight: 800 }}>{ct.key}</span>
                                        <span style={{ fontSize: 10, padding: "4px 10px", borderRadius: 4, background: mc(ct.mood) + "15", color: mc(ct.mood), fontWeight: 800 }}>{ct.mood}</span>
                                    </>
                                )}
                            </div>
                        </div>

                        {/* Circular Timer (Cronómetro) */}
                        <div style={{ position: "relative", width: 90, height: 90, flexShrink: 0 }}>
                            ...
                            <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                                <span style={{ fontSize: 18, fontWeight: 900, fontFamily: THEME.fonts.mono }}>{fmt(rem)}</span>
                                <span style={{ fontSize: 8, opacity: 0.4, textTransform: "uppercase" }}>restante</span>
                            </div>
                        </div>
                    </header>

                    {/* 2. BOTONES ACTIVADORES DE SUPERPODERES */}
                    <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
                        <button onClick={() => setCrossfade(!crossfade)} style={{ padding: "10px 16px", borderRadius: THEME.radius.md, border: `1px solid ${crossfade ? THEME.colors.brand.cyan : "rgba(255,255,255,0.1)"}`, background: crossfade ? THEME.colors.brand.cyan + "20" : "transparent", color: crossfade ? THEME.colors.brand.cyan : THEME.colors.text.muted, fontSize: 11, fontWeight: 900, cursor: "pointer" }}>CROSS</button>
                    ...
                    {/* 3. SECCIÓN DE CONTROLES (DASHBOARD) */}
                    <Dashboard
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, fontFamily: THEME.fonts.mono, opacity: 0.5 }}>
                            <span>{fmt(pos)}</span>
                            <span>-{fmt(rem)}</span>
                        </div>
                    </div>
                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 48 }}>
                        <button onClick={() => { if (!isLive && ci > 0) { setCi(ci - 1); setPos(0); } }} style={{ background: "none", border: "none", opacity: ci > 0 ? 0.9 : 0.2, cursor: "pointer" }}><svg width="32" height="32" viewBox="0 0 24 24" fill="white"><path d="M6 6h2v12H6zm3.5 6l8.5 6V6z" /></svg></button>
                        <button onClick={() => setPlaying(!playing)} style={{ width: 88, height: 88, borderRadius: "50%", border: "none", background: THEME.gradients.brand, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", boxShadow: playing ? `0 0 40px ${mCol}50` : "0 10px 30px rgba(0,0,0,0.5)" }}>{playing ? <svg width="36" height="36" viewBox="0 0 24 24" fill="white"><path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z" /></svg> : <svg width="36" height="36" viewBox="0 0 24 24" fill="white" style={{ marginLeft: 6 }}><path d="M8 5v14l11-7z" /></svg>}</button>
                        <button onClick={() => { if (!isLive && ci < pQueue.length - 1) { setCi(ci + 1); setPos(0); } }} style={{ background: "none", border: "none", opacity: ci < pQueue.length - 1 ? 0.9 : 0.2, cursor: "pointer" }}><svg width="32" height="32" viewBox="0 0 24 24" fill="white"><path d="M6 18l8.5-6L6 6v12zM16 6v12h2V6h-2z" /></svg></button>
                    </div>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 20, padding: "0 10px" }}>
                        ...
                        <div style={{ flex: 1, position: "relative", height: 10 }}>
                            ...
                            <input type="range" min="0" max="100" value={vol * 100} onChange={e => setVol(parseInt(e.target.value) / 100)} style={{ position: "absolute", inset: 0, width: "100%", height: "100%", opacity: 0, cursor: "pointer" }} />
                        </div>
                        <span style={{ fontSize: 16, fontWeight: 900, fontFamily: THEME.fonts.mono, color: mCol, width: 50 }}>{Math.round(vol * 100)}%</span>
                    </div>

                    {/* 7. DURACIÓN DEL SET Y ELAPSED */}
                    <div style={{ marginTop: "auto", padding: "24px", borderRadius: THEME.radius.xl, backgroundColor: THEME.colors.surface, border: `1px solid ${isLive ? THEME.colors.brand.cyan + "25" : THEME.colors.border}`, display: "flex", flexDirection: "column", gap: 20 }}>
                        <div style={{ width: "100%" }}>
                            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
                                <div style={{ display: "flex", flexDirection: "column" }}>
                                    <span style={{ fontSize: 9, opacity: 0.5, fontWeight: 900, letterSpacing: 1.5 }}>SET ELAPSED</span>
                                    <span style={{ fontSize: 18, fontWeight: 900, color: THEME.colors.brand.cyan, fontFamily: THEME.fonts.mono }}>{fmtM(elapsed * 1000)}</span>
                                </div>
                                <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end" }}>
                                    <span style={{ fontSize: 9, opacity: 0.5, fontWeight: 900, letterSpacing: 1.5 }}>TOTAL REMAINING</span>
                                    <span style={{ fontSize: 18, fontWeight: 900, color: mCol, fontFamily: THEME.fonts.mono }}>{fmtM(Math.max(0, qTot - (sumTrackDurationMs(pQueue.slice(0, ci)) + pos)))}</span>
                                </div>
                            </div>
                ...
                <div style={{ width: isMobile ? "100vw" : 360, maxWidth: 360, height: "100%", display: "flex", flexDirection: "column" }}>
                    <div style={{ padding: "24px", borderBottom: `1px solid ${THEME.colors.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        ...
                        <button 
                            onClick={() => setShowQueue(false)}
                            style={{ 
                                background: "none", border: "none", color: THEME.colors.text.muted, 
                                cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center",
                                width: 32, height: 32, borderRadius: "50%", transition: "all 0.2s"
                            }}
