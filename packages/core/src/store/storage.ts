...
const lazyStorage: IStorage & StateStorage = {
  ...
  // --- Backup Functions ---
  exportFullBackup: () => _impl?.exportFullBackup ? _impl.exportFullBackup() : Promise.reject("Backup not implemented on this platform"),
  importFullBackup: (backup: Blob) => _impl?.importFullBackup ? _impl.importFullBackup(backup) : Promise.reject("Import not implemented on this platform"),
};
