...
| 02-modelo-colecciones | [[00-vision-general]] | 00-vision-general, 01-modelo-audio, 10-algoritmo-mood (colecciones de curva), 03-vista-libreria (asignación a colecciones), 18-completador-set (items confirmados a la QuouList) | 01-audio-engine (fuente de reproducción), 01-vista-inicio (colecciones a mostrar), 05-vista-edit (sets), 04-almacenamiento, 08-modelo-jam-session (base de la cola compartida), 12-cronometro (duración objetivo del Set), 18-completador-set (canciones candidatas con duración efectiva) |
...
| 12-cronometro | [[03-modelo-sesion]] | 03-modelo-sesion (inicio/fin de modos), 02-modelo-colecciones (duración objetivo del Set para countdown) | 04-vista-show (crono ascendente + cuenta regresiva y alertas), 06-vista-perfil (estadísticas), 04-almacenamiento (historial_shows), 05-telemetria (tiempos acumulados), 18-completador-set (tiempo restante del show) |
...
| 15-sesion-audio (NUEVO) | [[03-modelo-sesion]] | sistema operativo (interrupciones, desconexión de salida, botones físicos, pedalera BT — físico), 03-modelo-sesion (política por modo), 06-vista-perfil (mapeo configurable de pedales) | 01-audio-engine (órdenes pausa/reanudar y comandos mapeados), 09-partituras (pasar página), 03-modelo-sesion (eventos que afectan al modo) |
...
| 18-completador-set (NUEVO) | [[04-vista-show]] | 12-cronometro (tiempo restante del show), 02-modelo-colecciones (canciones candidatas con duración efectiva), 04-vista-show (activación) | 04-vista-show (propuesta de tracks que suman el tiempo restante — el músico confirma), 02-modelo-colecciones (items confirmados a la QuouList) |
...
| 04-vista-show | [[05-vista-edit]] | 05-vista-edit (única entrada al modo Show), 03-modelo-sesion (reglas del modo), 12-cronometro (crono ascendente + cuenta regresiva y alertas), 08-mirror (visor), 09-partituras (visualización), 18-completador-set (propuesta de tracks para el tiempo restante), 13-tema | 01-audio-engine (órdenes limitadas + MUTE de pánico), 08-mirror (activación), 18-completador-set (activación del completador) |
...
| 06-vista-perfil | [[01-vista-inicio]] | 05-telemetria (estadísticas), 12-cronometro (tiempos), 14-sync-engine (estado), 06-modelo-backup-sync (opciones de configuración), 13-tema | 13-tema (selección de tema), 14-sync-engine (activación de backup), 04-almacenamiento (preferencias), 15-sesion-audio (mapeo configurable de pedales) |
...
- [ ] **Paso 2: Retrofitear los 13.** Particularidades:
  ...
  - `01-audio-engine.md` (Gap): precisar que el Gap es configurable en segundos con precisión de milisegundos (ej. 0.5s, 1.2s, 2s).
  - `12-cronometro.md`: agregar sección **Cuenta regresiva de set**: además del cronómetro ascendente de Show, un modo countdown con duración objetivo personalizable (típico: 45 o 90 min, tomada de la duración objetivo del Set o configurada al iniciar el Show) y alertas de hitos (faltan 10 min, faltan 5 min, tiempo cumplido — visuales, nunca sonoras durante un show). Nuevas aristas: ← [[02-modelo-colecciones]] (duración objetivo del Set), → [[18-completador-set]] (tiempo restante del show).
...
- [ ] **Paso 2: Retrofitear las 6.** Particularidades (decisiones del spec):
  ...
  - `04-vista-show.md`: agregar **MUTE de pánico** a los controles permitidos (actualizar wireframe y la lista de lo que SÍ se puede hacer); agregar la **cuenta regresiva del set** junto al crono ascendente (con alertas visuales de hitos) y el acceso al **completador de set** ("Completar tiempo restante" — muestra la propuesta de [[18-completador-set]] y el músico confirma).
  - `06-vista-perfil.md`: agregar sección de **configuración de pedalera Bluetooth**: sincronizar pedalera, y por cada pedal (típicamente 2) asignar un comando de una lista (siguiente, anterior, play/pausa, mute, pasar página de partitura, crear marcador). Salida → [[15-sesion-audio]].
...
**Files (Create):** `docs/especificaciones/06-modelo-backup-sync.md`, `07-modelo-errores.md`, `08-modelo-jam-session.md`; `docs/componentes/14-sync-engine.md`, `15-sesion-audio.md`, `16-ecualizador.md`, `17-jam-session.md`, `18-completador-set.md`; `docs/diccionario/00-diccionario-dominio.md`, `01-diccionario-datos.md`; `docs/funciones/00-catalogo-funciones.md`
**Files (Modify):** `docs/INDEX.md`

- [ ] **Paso 1: Crear los 8 docs de producto** con contrato completo según el Mapa de Aristas y el contenido descripto en la Sección 2 del spec. Los 2 de Jam Session con `estado: borrador` y banner FASE 2. `07-modelo-errores.md` cataloga los errores por categoría (archivo, formato, procesamiento, sistema, sincronización, datos) y linkea las secciones Errores de cada doc. Particularidades:
  - `15-sesion-audio.md`: incluir la **configuración de pedalera**: flujo de sincronización BT, mapeo por pedal (el usuario presiona un pedal y elige el comando de una lista — típicamente 2 pedales, totalmente personalizables), persistencia del mapeo. Errores: pedalera desvinculada a mitad de show (los pedales dejan de responder, NUNCA disparan comandos fantasma), comando asignado dos veces.
  - `18-completador-set.md`: el completador de tiempo restante: recibe el tiempo faltante de [[12-cronometro]], busca combinaciones de canciones candidatas cuya **duración efectiva** (fin personalizado − inicio personalizado) sume ese tiempo dentro de una tolerancia configurable (ej. ±30s), excluye las ya reproducidas en el show, y PROPONE — el músico confirma antes de que entre a la QuouList (cero sorpresas en vivo). Errores: ninguna combinación alcanza la tolerancia (ofrece la más cercana), librería insuficiente, tiempo restante menor que la canción más corta.
